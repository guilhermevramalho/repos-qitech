...
class TestHealthCheck:
    def test_home(self):
        response = ClientRequisition.send("GET", "/api_boilerplate/")
        assert response.response_status == 200

    def test_health_check(self):
        response = ClientRequisition.send("GET", "/api_boilerplate/health_check")
        assert response.response_status == 204

    def test_no_token(self):
        response = ClientRequisition.send("PUT", "/api_boilerplate/sample")
    ...
    def test_method_not_allowed(self):
        response = ClientRequisition.send("PUT", "/api_boilerplate/health_check")
